...
class AccountOfficerController extends Controller
{
    public function update(Request $request, $profileId)
    {
        ...
        // Only allow profiles that are Account Officers (roles_id 7)
        // previously this controller was copied from loan-officer logic.
        if ($profile->roles_id !== 7) {
            return response()->json([
                'error' => 'Unauthorized',
        ...
        // Validate fields for both Profile and User. When updating an existing record we
        // must allow the current profile's email (and username on the related user) to
        // remain the same without triggering the unique constraint.
        $validated = $request->validate([
            ...
            'email' => [
                ...
                'email',
                'max:255',
                // ignore the record being updated (profile_id is the primary key)
                'unique:profiles,email,'.$profileId.',profile_id',
            ],
