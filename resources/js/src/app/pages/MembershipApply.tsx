...
// ─── FIX 1: StepIndicator — compact labels, scrollable, no overflow ───
          ...
          return (
            <div key={i} className="flex items-center">
              <div className="flex flex-col items-center w-16 sm:w-24">
                <div
                  className={`
                    w-9 h-9 sm:w-10 sm:h-10 rounded-full flex items-center justify-center font-bold text-xs sm:text-sm border-2 transition-all duration-300 flex-shrink-0
                ...
                <span className={`mt-1.5 text-[10px] sm:text-xs font-semibold text-center leading-tight px-0.5 sm:hidden ${active ? 'text-primary' : 'text-muted-foreground'}`}>
                  {label}
                </span>
                <span className={`mt-1.5 text-xs font-semibold text-center leading-tight hidden sm:block max-w-[90px] ${active ? 'text-primary' : 'text-muted-foreground'}`}>
                  {fullLabels[i]}
                </span>
              </div>
              {i < steps.length - 1 && (
                <div className={`w-6 sm:w-14 h-0.5 mb-5 flex-shrink-0 transition-all duration-500 ${done ? 'bg-primary' : 'bg-border'}`} />
              )}
            </div>
          );
        })}
    ...
    <div className="space-y-1.5">
      ...
      <div
        ...
        className={`
          ...
          cursor-pointer min-h-[90px] px-4 py-5 text-center transition-colors
          ${dragging ? 'border-primary bg-primary/5' : 'border-border bg-muted/30 hover:bg-muted/50'}
        `}
      >
        <Upload className="w-5 h-5 text-muted-foreground" />
        {fileName ? (
          <span className="text-sm text-primary font-medium break-all">{fileName}</span>
        ) : (
          <span className="text-sm text-muted-foreground">
            Tap to <span className="text-primary font-medium">Browse</span>
          </span>
        )}
        {helperText && <span className="text-xs text-muted-foreground mt-0.5">{helperText}</span>}
        <input
          ref={inputRef}
          type="file"
...
export function MembershipApply() {
    ...
    try {
      ...
      if (draft.idFileName) {
        getFileFromDb('id_file_front').then((file) => {
          if (file) setIdFileFront(file);
        });
        getFileFromDb('id_file_back').then((file) => {
          if (file) setIdFileBack(file);
        });
      }
    } catch {
  ...
  useEffect(() => {
    ...
    step, selectedTypeId, profileData, applicationData, spouseData, coMakersData,
    orientationProgress, watchedProfile, watchedApplication, watch3,
    idFileFront, idFileBack, saveFileToDb,
  ]);
  ...
  const submitFinalApplication = async () => {
    ...
    try {
      ...
      Object.entries(profileData).forEach(([k, v]) => {
        if (v && String(v).trim()) formData.append(k, String(v));
      });
      ...
      if (spouseData && spouseData.full_name && spouseData.full_name.trim()) {
        Object.entries(spouseData).forEach(([k, v]) => {
          if (v && String(v).trim()) formData.append(`spouse_${k}`, String(v));
        });
      }
  ...
  if (submitted) {
    return (
      <div className="flex flex-col">
        ...
        <section className="py-16 sm:py-28 flex items-center justify-center px-4">
  ...
  return (
    // FIX: add scrollbar-hide utility via style tag
    <>
      ...
      <div className="flex flex-col">
        ...
        <section className="relative py-10 sm:py-24 overflow-hidden">
          ...
          <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {/* FIX 3: Badge + clear draft — stack on mobile */}
            <div className="flex flex-col sm:flex-row items-center justify-center gap-3 mb-6">
              <span className="inline-flex items-center gap-2 bg-primary/10 border border-primary/20 text-primary text-sm font-semibold px-4 py-2 rounded-full text-center">
              ...
              <form onSubmit={handle1(submitProfile)}>
                <Card className="rounded-2xl bg-card/90 backdrop-blur-sm border-border/50 shadow-sm">
                  ...
                  <CardContent className="space-y-5 pt-6">
                    ...
                    {/* FIX 4: Name fields — 1 col on mobile, 3 on sm+ */}
                    ...
                    {/* FIX 5: Birthdate/sex/civil status — 1 col on mobile, 3 on sm */}
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      <div className="space-y-1.5">
                ...
                {/* FIX 6: Nav — full width on mobile */}
                <div className="flex justify-end mt-6">
                  <Button type="submit" className="w-full sm:w-auto bg-primary hover:bg-primary/90 text-white rounded-lg px-8 font-medium">
                    Save & Continue
                  </Button>
                </div>
              </form>
            ...
            {step === 2 && (
              <form onSubmit={handle2(saveApplicationStep)}>
                {/* FIX 7: Documents card moves ABOVE application card on mobile */}
                <div className="flex flex-col lg:flex-row gap-6">
                  {/* Documents card — shown first on mobile via order */}
                ...
                {/* FIX 8: Nav — stack on mobile, truncate long label */}
                <div className="flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-between gap-3 mt-6">
                  ...
                  <Button type="submit" className="bg-primary hover:bg-primary/90 text-white rounded-lg px-6 font-medium">
                    Save & Continue
                  </Button>
                </div>
              </form>
                        ...
                        <div key={idx} className="border border-border rounded-lg p-4 space-y-3">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <div className="space-y-1.5">
                              ...
                              <Input value={coMaker.full_name} onChange={(e) => { const u = [...coMakersData]; u[idx].full_name = e.target.value; setCoMakersData(u); }} className="rounded-lg text-sm" />
                            </div>
                            <div className="space-y-1.5">
                              <Label className="text-sm">Relationship</Label>
                              <Input value={coMaker.relationship} onChange={(e) => { const u = [...coMakersData]; u[idx].relationship = e.target.value; setCoMakersData(u); }} className="rounded-lg text-sm" />
                            </div>
                          </div>
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <div className="space-y-1.5">
                              ...
                              <Input type="tel" value={coMaker.contact_number} onChange={(e) => { const u = [...coMakersData]; u[idx].contact_number = e.target.value; setCoMakersData(u); }} className="rounded-lg text-sm" />
                            </div>
                            <div className="space-y-1.5">
                              <Label className="text-sm">Occupation</Label>
                              <Input value={coMaker.occupation} onChange={(e) => { const u = [...coMakersData]; u[idx].occupation = e.target.value; setCoMakersData(u); }} className="rounded-lg text-sm" />
                            </div>
                          </div>

                          <div className="space-y-1.5">
                            <Label className="text-sm">Address</Label>
                            <Input value={coMaker.address} onChange={(e) => { const u = [...coMakersData]; u[idx].address = e.target.value; setCoMakersData(u); }} className="rounded-lg text-sm" />
                          </div>
                          <div className="space-y-1.5">
                            <Label className="text-sm">Employer</Label>
                            <Input value={coMaker.employer_name} onChange={(e) => { const u = [...coMakersData]; u[idx].employer_name = e.target.value; setCoMakersData(u); }} className="rounded-lg text-sm" />
                          </div>
                          <div className="space-y-1.5">
                            <Label className="text-sm">Monthly Income (₱)</Label>
                            <Input type="number" min={0} value={coMaker.monthly_income} onChange={(e) => { const u = [...coMakersData]; u[idx].monthly_income = e.target.value; setCoMakersData(u); }} className="rounded-lg text-sm" />
                          </div>
                        </div>
                      ))}
              ...
              <div className="space-y-6">
                <Card className="rounded-2xl bg-card/90 backdrop-blur-sm border-border/50 shadow-sm">
                  ...
                  <CardContent className="space-y-6 pt-6">
                    {/* FIX 10: Orientation cards — stack on mobile, side-by-side on sm+ (not md) */}
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className="rounded-xl border border-border p-4">
                        ...
                        {orientationSettings.zoom_link ? (
                          <button
                            ...
                            className="inline-flex items-center justify-center w-full rounded-lg bg-primary text-white py-3 font-medium hover:bg-primary/90 transition-colors text-sm"
                          >
                            {zoomClicked ? '✓ Zoom Link Accessed' : 'Join Zoom Orientation'}
                          </button>
                        ) : (
                          <div className="w-full rounded-lg border bg-muted flex items-center justify-center text-sm text-muted-foreground px-4 py-6 text-center">
                      ...
                      <div className="rounded-xl border border-border p-4">
                        ...
                        <p className="text-sm text-muted-foreground mb-3">
                          Watch the orientation video completely.
                        </p>
                        ...
                        {orientationSettings.video_link ? (
                          <div className="space-y-3">
                            ...
                            {videoInteracted ? (
                              <div className="w-full rounded-lg bg-green-50 border border-green-200 px-3 py-2.5 text-center text-sm text-green-700 font-medium">
                                ✓ Video watched completely
                              </div>
                            ) : (
                              <div className="w-full rounded-lg bg-amber-50 border border-amber-200 px-3 py-2.5 text-center text-xs text-amber-700">
                                Watch until the end to mark as watched
                              </div>
                            )}
                          </div>
                        ) : (
                          <div className="w-full h-40 rounded-lg border bg-muted flex items-center justify-center text-sm text-muted-foreground px-4 text-center">
                    ...
                    <div className="rounded-xl border border-border p-4">
                      <div className="flex items-center gap-2 mb-2">
                        <FileBadge className="w-5 h-5 text-primary flex-shrink-0" />
                            ...
                            <Select
                              onValueChange={(value) => {
                                if (!assessmentSubmitted) {
                                  setAssessmentAnswers((prev) => ({ ...prev, [index]: value }));
                                }
                              }}
                    ...
                    {/* FIX 11: Checklist — better mobile layout */}
                    <div className="rounded-xl border border-primary/20 bg-primary/5 p-4">
                      <h3 className="font-semibold mb-3 text-sm">Orientation Checklist</h3>
                      <div className="space-y-2.5">
                        {[
                          ...
                          <div key={label} className="flex items-center justify-between gap-2 text-sm">
                            ...
                            <span className={`flex-shrink-0 font-medium text-xs ${done ? 'text-green-600' : failed ? 'text-destructive' : 'text-muted-foreground'}`}>
                ...
                {/* FIX 12: Final nav — stack on mobile */}
...
// FIX 13: Hero — reduce height on mobile, center-align text on small screens
function HeroSection({
  ...
  return (
    <section className="relative text-white py-14 sm:py-24 lg:py-32 overflow-hidden">
      <div
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Center on mobile, left-align on sm+ */}
        <div className="max-w-3xl mx-auto sm:mx-0 text-center sm:text-left">
          ...
          <h1 className="text-3xl sm:text-5xl lg:text-6xl font-bold mb-4 leading-tight">Membership Application</h1>
          {membershipTypeLabel && (
